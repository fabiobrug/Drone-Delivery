...
const SystemConfig = () => {
  const { config, updateConfig, noFlyZones, addNoFlyZone, removeNoFlyZone } = useDroneContext()
  ...
  return (
    <div className="h-full p-6 overflow-y-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-400">Ajuste os parâmetros gerais e regras da simulação de entregas</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Optimization Parameters */}
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-white mb-4">Parâmetros de Otimização</h3>
          ...
          <div className="space-y-4">
            <div>
              <label className="flex items-center space-x-3">
                <input
                  ...
                  onChange={(e) => handleConfigChange("optimizationEnabled", e.target.checked)}
                  className="w-4 h-4 text-blue-600 bg-gray-900 border-gray-600 rounded focus:ring-blue-500 focus:ring-2"
                />
                <span className="text-sm font-medium text-gray-300">Habilitar Otimização Inteligente</span>
              </label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Método de Otimização</label>
              <select
              ...
              >
                <option value="priority-distance-weight">Prioridade + Distância + Peso</option>
                <option value="priority-only">Somente Prioridade</option>
                <option value="distance-only">Somente Distância</option>
              </select>
              <p className="text-xs text-gray-400 mt-1">Critério usado para otimizar a alocação de pedidos</p>
            </div>

            <div className="bg-gray-900 rounded-lg p-4">
              <h4 className="text-sm font-medium text-white mb-2">Status da Otimização</h4>
              <div className="flex items-center space-x-2">
                <div
                  className={`w-2 h-2 rounded-full ${config.optimizationEnabled ? "bg-green-500" : "bg-gray-500"}`}
                ></div>
                <span className="text-sm text-gray-300">{config.optimizationEnabled ? "Ativa" : "Desativada"}</span>
              </div>
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              />
              <p className="text-xs text-gray-400 mt-1">Frequência de atualização dos dados em tempo real</p>
            </div>
            ...
            <div>
              <label className="flex items-center space-x-3">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 bg-gray-900 border-gray-600 rounded focus:ring-blue-500 focus:ring-2"
                />
                <span className="text-sm font-medium text-gray-300">Notificações em Tempo Real</span>
              </label>
              <p className="text-xs text-gray-400 mt-1 ml-7">Receber alertas sobre status dos drones e pedidos</p>
            </div>
            ...
            <div>
              <label className="flex items-center space-x-3">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 bg-gray-900 border-gray-600 rounded focus:ring-blue-500 focus:ring-2"
                />
                <span className="text-sm font-medium text-gray-300">Log de Atividades</span>
              </label>
              <p className="text-xs text-gray-400 mt-1 ml-7">Registrar todas as ações do sistema para auditoria</p>
            </div>
          </div>
        </div>
        ...
        {/* No-Fly Zones */}
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-white mb-4">Zonas de Exclusão Aérea</h3>
          ...
          <div className="space-y-2 max-h-40 overflow-y-auto">
            {noFlyZones.map((zone) => {
              ...
              return (
                <div key={zone.id} className="flex items-center justify-between bg-gray-900 rounded-lg p-3">
            ...
            {noFlyZones.length === 0 && (
              <div className="text-center text-gray-400 text-sm py-4">Nenhuma zona de exclusão definida</div>
            )}
          </div>
        </div>
        ...
        {/* Simulation Control */}
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-white mb-4">Controle da Simulação</h3>

          <div className="space-y-4">
            <div className="bg-gray-900 rounded-lg p-4">
              <h4 className="text-sm font-medium text-white mb-2">Status Atual</h4>
              <div className="flex items-center space-x-2">
                <div
                  className={`w-3 h-3 rounded-full ${isSimulationRunning ? "bg-green-500 animate-pulse" : "bg-gray-500"}`}
                ></div>
            ...
            >
              ...
              <span>{isSimulationRunning ? "Parar Simulação" : "Iniciar Simulação"}</span>
            </button>
